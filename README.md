<!-- ████████████████████████████████████████████████████████████ -->
<!--          🌌  MILKY WAY EDITION — AAYUSH KUMAR  🌌           -->
<!-- ████████████████████████████████████████████████████████████ -->

<div align="center">

<!-- ✦ DEEP SPACE HEADER — GALAXY NEBULA BURST -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=venom&color=0:000000,20:03001C,50:0B0C10,80:1C1C3C,100:000000&height=240&section=header&text=AAYUSH%20KUMAR&fontSize=58&fontColor=E0FBFC&animation=fadeIn&fontAlignY=38&desc=✦%20%20Drifting%20Through%20the%20MERN%20Galaxy%20%20✦&descAlignY=62&descSize=17&descColor=9D4EDD&stroke=9D4EDD&strokeWidth=1"/>

<br/>

<!-- ✦ ORBITRON TYPING — SPACE CONSOLE -->
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=20&pause=1500&color=9D4EDD&background=00000000&center=true&vCenter=true&random=false&width=700&height=50&lines=Somewhere+between+Delhi+and+Andromeda...;400+Billion+Stars.+Infinite+Lines+of+Code.;Final+Year+BCA+%7C+MERN+Stack+Explorer;Compiling+Dreams+into+Full-Stack+Apps;One+Commit+Closer+to+the+Event+Horizon;Open+to+Internships+%26+Cosmic+Collabs" alt="Typing SVG" />

<br/><br/>

<!-- ✦ HOLOGRAPHIC BADGE ROW -->
<img src="https://img.shields.io/badge/◈_CLASS-FULL__STACK__DEV-9D4EDD?style=for-the-badge&labelColor=03001C"/>
<img src="https://img.shields.io/badge/◈_SECTOR-MERN__GALAXY-00B4D8?style=for-the-badge&labelColor=03001C"/>
<img src="https://img.shields.io/badge/◈_STATUS-OPEN__TO__WORK-39FF14?style=for-the-badge&labelColor=03001C"/>

<br/><br/>

<!-- ✦ COSMIC COUNTERS -->
<img src="https://komarev.com/ghpvc/?username=Pro472&label=🛸+COSMIC+VISITORS&color=9D4EDD&style=for-the-badge&labelColor=03001C"/>
&nbsp;
<img src="https://img.shields.io/badge/☕_CHAI-∞_CONSUMED-F4A261?style=for-the-badge&labelColor=03001C"/>

</div>

<br/>

<!-- ✦ STARDUST DIVIDER -->
<div align="center">
<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="900"/>
</div>

<br/>

<!-- ████████████████████████████████████████████████████████████ -->
<!--                  🛸  PILOT PROFILE BOOT                     -->
<!-- ████████████████████████████████████████████████████████████ -->

<div align="center">

## 🌌 &nbsp;`[ PILOT_PROFILE :: BOOTING... ]`

<img src="https://user-images.githubusercontent.com/74038190/229223263-cf2e4b07-2615-4f87-9c38-e37600f8381a.gif" width="380"/>

</div>

<br/>

```python
# ╔══════════════════════════════════════════════════════════════╗
# ║         ✦  DEEP SPACE ENTITY DETECTED  ✦                    ║
# ║         TRIANGULATING COORDINATES...   🛰️                   ║
# ╚══════════════════════════════════════════════════════════════╝

class CosmicDeveloper:

    identity     = "Aayush Kumar"
    coordinates  = "Delhi, India 🌏  →  28.6139°N, 77.2090°E"
    stardate     = "Final Year BCA — Class of 2025 🎓"
    faction      = "MERN Stack Developer ⚡"

    # ── ACTIVE MISSIONS ──────────────────────────────────────────
    warp_to      = "Get Me A Chai ☕  (Crowdfunding Platform)"
    nav_systems  = ["Next.js 14", "MongoDB Atlas", "React 18", "Node.js"]
    endgame      = "Patreon Clone — Full MERN Architecture 🎯"

    # ── COMPLETED MISSIONS ───────────────────────────────────────
    past_ops     = ["DelhiRide 🚗 — PHP + MySQL Vehicle Rental System"]
    classified   = "Built an entire RPG game as a love letter 💘"

    # ── PILOT VITALS ─────────────────────────────────────────────
    fuel         = "Chai  ☕ × ∞"
    debug_mode   = True   # always
    sleep_cycles = None   # who needs those
    open_to      = ["Internships 🚀", "Collaborations 🛸", "Wild Ideas 💡"]

    def transmit(self):
        return "aayushkumar@email.com  📡"
```

<br/>

<!-- ✦ GALAXY DIVIDER -->
<div align="center">
<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="900"/>
</div>

<br/>

<!-- ████████████████████████████████████████████████████████████ -->
<!--              🪐  TECH CONSTELLATION MAP                     -->
<!-- ████████████████████████████████████████████████████████████ -->

<div align="center">

## 🪐 &nbsp;`[ TECH_CONSTELLATION :: WEAPONS_ARRAY ]`

<img src="https://skillicons.dev/icons?i=html,css,js,react,nextjs,nodejs,express,mongodb,mysql,php,git,github,vscode,vercel,postman&theme=dark&perline=8" />

<br/>

</div>

<br/>

<!-- ✦ NEBULA WAVE DIVIDER -->
<div align="center">
<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="900"/>
</div>

<br/>

<!-- ████████████████████████████████████████████████████████████ -->
<!--              📡  DEEP SPACE ANALYTICS                       -->
<!-- ████████████████████████████████████████████████████████████ -->

<div align="center">

## 📡 &nbsp;`[ DEEP_SPACE_SCAN :: GALACTIC_ANALYTICS ]`

<img src="https://user-images.githubusercontent.com/74038190/225023900-4667b6ac-0ce6-4c2b-8b7e-37562eb8da4b.gif" width="120"/>

<br/><br/>

<img width="49%" src="https://github-readme-stats.vercel.app/api?username=Pro472&show_icons=true&theme=tokyonight&hide_border=true&count_private=true&bg_color=03001C&title_color=9D4EDD&icon_color=00B4D8&text_color=E0FBFC&border_radius=16" />
<img width="49%" src="https://streak-stats.demolab.com/?user=Pro472&theme=tokyonight&hide_border=true&background=03001C&stroke=9D4EDD&ring=00B4D8&fire=F4A261&currStreakLabel=9D4EDD&sideLabels=E0FBFC&dates=888888&border_radius=16" />

<br/><br/>

<img width="43%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Pro472&layout=compact&theme=tokyonight&hide_border=true&bg_color=03001C&title_color=9D4EDD&text_color=E0FBFC&langs_count=8&border_radius=16" />

<br/><br/>

<img width="97%" src="https://github-readme-activity-graph.vercel.app/graph?username=Pro472&bg_color=03001C&color=9D4EDD&line=00B4D8&point=E0FBFC&area=true&area_color=9D4EDD&hide_border=false&border_color=9D4EDD&radius=8&custom_title=✦%20COSMIC%20CONTRIBUTION%20MAP%20✦" />

</div>

<br/>

<!-- ✦ STARDUST DIVIDER -->
<div align="center">
<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="900"/>
</div>

<br/>

<!-- ████████████████████████████████████████████████████████████ -->
<!--              🏆  STELLAR ACHIEVEMENT VAULT                  -->
<!-- ████████████████████████████████████████████████████████████ -->

<div align="center">

## 🌟 &nbsp;`[ MISSION_LOG :: STELLAR_ACHIEVEMENTS ]`

<br/>

<img src="https://github-profile-trophy.vercel.app/?username=Pro472&theme=radical&no-frame=true&margin-w=10&row=1&column=7" />

</div>

<br/>

<!-- ✦ GALAXY DIVIDER -->
<div align="center">
<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="900"/>
</div>

<br/>

<!-- ████████████████████████████████████████████████████████████ -->
<!--              🚀  ACTIVE MISSION DOSSIERS                    -->
<!-- ████████████████████████████████████████████████████████████ -->

<div align="center">

## 🚀 &nbsp;`[ MISSION_CONTROL :: ACTIVE_DOSSIERS ]`

</div>

<br/>

<table align="center" width="100%" border="0">
<tr>
<td width="50%" valign="top" align="center">

### ☕ &nbsp; GET ME A CHAI
```
┌─────────────────────────────────┐
│  MISSION STATUS : 🟢 LAUNCHING  │
│  STACK   : Next.js + MongoDB    │
│  TYPE    : Crowdfunding App     │
│  PHASE   : Active Development   │
└─────────────────────────────────┘
```
*Where creators meet their believers.*

</td>
<td width="50%" valign="top" align="center">

### 🚗 &nbsp; DELHIRIDE
```
┌─────────────────────────────────┐
│  MISSION STATUS : ✅ COMPLETE   │
│  STACK   : PHP + MySQL          │
│  TYPE    : Vehicle Rental SYS   │
│  PHASE   : Deployed             │
└─────────────────────────────────┘
```
*Delhi roads. Digital wheels.*

</td>
</tr>

</table>

<table align="center" width="55%" border="0">
<tr>
<td valign="top" align="center">

### 💘 &nbsp; LOVE QUEST RPG
```
┌─────────────────────────────────┐
│  MISSION STATUS : ✅ COMPLETE   │
│  STACK   : Vanilla JS + Canvas  │
│  TYPE    : Pixel Art RPG Game   │
│  PHASE   : Open Source 🌌       │
└─────────────────────────────────┘
```
*A 3-level game. One real story.*

</td>
</tr>
</table>

<br/>

<!-- ✦ NEBULA DIVIDER -->
<div align="center">
<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="900"/>
</div>

<br/>

<!-- ████████████████████████████████████████████████████████████ -->
<!--              🌠  QUANTUM SNAKE TRAIL                        -->
<!-- ████████████████████████████████████████████████████████████ -->

<div align="center">

## 🌠 &nbsp;`[ QUANTUM_TRAIL :: NEBULA_CONTRIBUTIONS ]`

*Every green square is a star I placed in this galaxy.*

<br/>

<!-- ✦ This contribution chart works with ZERO setup needed -->
<img width="100%" src="https://ghchart.rshah.org/9D4EDD/Pro472" alt="Aayush's Contribution Galaxy" />

<br/><br/>

<!-- ✦ OPTIONAL: Want the animated snake instead? -->
<!-- Set up the GitHub Action from: https://github.com/Platane/snk -->
<!-- Once set up, replace the above with this line: -->
<!-- <img src="https://raw.githubusercontent.com/Pro472/Pro472/output/github-contribution-grid-snake-dark.svg" /> -->

</div>

<br/>

<!-- ✦ STARDUST DIVIDER -->
<div align="center">
<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="900"/>
</div>

<br/>

<!-- ████████████████████████████████████████████████████████████ -->
<!--              📡  INTERSTELLAR COMMS                         -->
<!-- ████████████████████████████████████████████████████████████ -->

<div align="center">

## 📡 &nbsp;`[ OPEN_CHANNEL :: INTERSTELLAR_COMMS ]`

*Signal range: infinite. Response time: faster than light (usually).*

<br/>

<a href="https://linkedin.com/in/YOUR-LINKEDIN">
<img src="https://img.shields.io/badge/◈_LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=03001C"/>
</a>

<br/><br/>

<img src="https://user-images.githubusercontent.com/74038190/212284158-e840e285-664b-44d7-b79b-e264b5e54825.gif" width="100"/>

<br/>

```
╔══════════════════════════════════════════════════════════════════╗
║                                                                  ║
║   "The Milky Way has 400 billion stars.                          ║
║    I'm just trying to be the one that ships clean code           ║
║    and doesn't break production."                                ║
║                                                                  ║
║                              — Aayush Kumar  🌌                 ║
║                                                                  ║
╚══════════════════════════════════════════════════════════════════╝
```

<br/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=14&pause=2500&color=9D4EDD&center=true&vCenter=true&random=false&width=550&lines=Star+my+repos+if+they+helped+you;The+universe+is+written+in+JavaScript;Lets+build+something+out+of+this+world;See+you+at+the+event+horizon" alt="Footer Typing" />

<br/><br/>

<img width="100%" src="https://capsule-render.vercel.app/api?type=venom&color=0:000000,20:03001C,60:1C1C3C,100:000000&height=160&section=footer&text=TRANSMISSION%20END%20🛸&fontSize=26&fontColor=9D4EDD&animation=fadeIn&fontAlignY=55"/>

</div>
